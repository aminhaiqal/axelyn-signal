...
const SECRET_NAME = "openrouter_api_key";

export const OpenRouterApiKeySchema = z.string()
  .trim()
  .min(20, "Enter a complete OpenRouter API key.")
...
function displayHint(apiKey: string): string {
  const prefix = apiKey.startsWith("sk-or-") ? "sk-or-" : "key-";
  return `${prefix}••••${apiKey.slice(-4)}`;
}

export class OpenRouterKeyStore {
  async status(): Promise<OpenRouterKeyStatus> {
  ...
  async saveApiKey(rawApiKey: string): Promise<OpenRouterKeyStatus> {
    const apiKey = OpenRouterApiKeySchema.parse(rawApiKey);
  ...
  async deleteApiKey(): Promise<OpenRouterKeyStatus> {
    await ensureDatabase();
